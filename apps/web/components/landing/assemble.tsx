...
export function AssembleText({
  ...
  return (
    <div ref={ref} className={cn("relative h-[150vh]", className)}>
...
function Icon({
  ...
  const rotate = useTransform(progress, [0, 0.6], [d * 45, 0]);
  const scale = useTransform(progress, [0, 0.6], [0.7, 1]);
  return (
    <motion.li
      title={name}
      style={reduced ? undefined : { x, y, rotate, scale }}
      className="group grid size-14 place-items-center rounded-2xl border border-edge bg-surface text-ink-muted shadow-[0_10px_30px_-10px_rgb(0_0_0/0.8)] transition-colors duration-[--duration-normal] hover:border-brand/60 hover:text-ink sm:size-20"
    >
