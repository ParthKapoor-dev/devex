...
export default function NotAPlayground() {
  // Click-through: the headline track is pulled up over the hero with a
  // negative margin, so the section must not catch the hero's clicks. The
  // grid opts back in. See assemble.tsx.
  return (
    <section aria-labelledby="not-a-playground" className="pointer-events-none relative">
      ...
      <div className="pointer-events-auto relative mx-auto -mt-[22vh] max-w-6xl px-4 sm:px-6">
        ...
        <ul className="grid border-y border-edge bg-canvas/60 sm:grid-cols-2 sm:overflow-hidden sm:rounded-2xl sm:border sm:backdrop-blur-[2px] lg:grid-cols-4">
          {TRUTHS.map((t, i) => (
            <li
              key={t.k}
              className="group relative flex flex-col border-edge py-6 not-last:border-b sm:min-h-60 sm:p-7 sm:[&:nth-child(odd)]:border-r lg:border-b-0 lg:not-last:border-r"
            >
              <span
                aria-hidden="true"
              ...
              <h3 className="mt-auto pt-3 font-display text-2xl font-medium tracking-[-0.03em] text-ink sm:pt-10">
                {t.title}
              </h3>
