...
/**
 * The amber CRT behind the landing hero.
 *
 * React Bits "Faulty Terminal" (https://reactbits.dev, MIT + Commons Clause),
 * with three changes: `dpr` is read inside the effect so the module can be
 * imported on the server; array props are hoisted so they do not rebuild the
 * GL context; and `pause` stops the rAF loop through a ref instead of sitting
 * in the effect's dependencies, which used to tear the context down and
 * rebuild it on every toggle while the paused loop kept drawing the same
 * frame at 60fps.
 */
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    const dpr = dprProp ?? Math.min(window.devicePixelRatio || 1, 1.5);
    ...
    const update = (t: number) => {
      ...
      // Paused: this frame stays on screen and the loop stops until kicked.
